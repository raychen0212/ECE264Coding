  ...
  for (index = 0; index < size; index++)
    {
      ...
      for (update_index = (index + 1); update_index < (size + 1); update_index ++)
	{
	  if (arr[update_index] < arr[original_index])
	    {
	...
	{
	  small_num = arr[index];
	  arr[index] = arr[original_index];
          arr[original_index] = small_num;
	}
    }
